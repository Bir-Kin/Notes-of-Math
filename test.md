...
<figure>
<img src="Figures/SpernerLemma.jpg" />
...
固定染色 $\lambda$, 取重心重分 $K^0,K^1,\dots$, 则在每一个 $K^j$ 中
...
因此链映射 $$C_*(B,A\cap B)\rightarrow C_*^{\mathfrak{U}}(X)/C_*(A)$$
诱导了同调群之间的同构, 原命题成立. ◻
:::
...
我们有嵌入映射 $\iota:C^{\mathfrak{U}}_n(X)\hookrightarrow C_n(X)$,
